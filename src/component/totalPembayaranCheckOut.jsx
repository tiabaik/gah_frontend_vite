...
import DetailBayarFO from './detailBayarFO';



function TotalPembayranCheckOut(props) {
   const [modalShowB, setModalShowB] = useState(false);
    const [fasilitas, setFasilitas]=useState([])
    const [kamar, setKamar]= useState([])
    const [uangJD, setuangJD]= useState([])
    ...
    >
      ...
      <Modal.Body  >
        ...
        <Card className='p-2 m-3' >
              ...
              <div className="row mb-3">
                                    {
                                            uangJD.uang_jaminan != null ?
                                            <div className="col-sm fs-6">
                                              Jaminan :Rp.{uangJD.uang_jaminan.toLocaleString()}
                                            </div>
                                            : 
                                            <div className="col-sm fs-6">
                                              Jaminan :Rp.0
                                    ...
                                    {
                                            uangJD.uang_deposit != null ?
                                            <div className="row mb-3">
                                            <div className="col-sm fs-6 text-end">
                                              Deposit: Rp.{uangJD.uang_deposit.toLocaleString()}
                                            </div>
                                            <div className="col-sm fs-6 text-end">
                                              Sisa Deposit: Rp.{(uangJD.uang_deposit - totalF).toLocaleString()}
                                            </div>
                                          </div>                    
                                            : 
                                            <div className="col-sm fs-6">
      ...
      <DetailBayarFO
                id={idB}
                totalAkhir={(total+totalF)+((total+totalF) * 0.10)}
                tax={(total+totalF) * 0.10 }
                totalPembayaran={(total+totalF)}
                sisaPembayaran={(uangJD.uang_jaminan + uangJD.uang_deposit) - (total+totalF) - ((total+totalF) * 0.10) }
                show={modalShowB}
...
export default TotalPembayranCheckOut